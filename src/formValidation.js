...
function checkZipCode() {
  const constraints = {
    us: {
      regex: /^\d{5}-?(\d{4})?$/,
      errorMessage:
        "US ZIPs must be either five digits or five digits followed a hyphen(dash) and another four digits. e.g. 12345-123",
    },
    jp: {
      regex: /^\d{3}-?\d{4}$/,
    ...
    ca: {
      ...
      errorMessage:
        "Canada ZIPs must be six-character string consisiting of three lettes and three digits separated with a space e.g A1B 2C3",
    },
    nl: {
      regex: /^(NL-)?\d{4}\s*(A-RT-Z][A-Z]|S[BCE-RT-Z])$/,
      errorMessage:
        "Netherland ZIPs must have exactly 4 digits, followed by 2 letter except SA, SD and SS. e.g. NL-1234 SF",
    },
  };
...
function checkPassword() {
  ...
  const errorMessage =
    "Password must be at least 6 charaters long. It must contain number, lower case alphabet, upper case alphabet and symbol.";
...
function checkConfirmPassword() {
  ...
  const passwordMatch = passwordInput === confirmationPassword.value;

  if (passwordMatch) {
    confirmationPassword.setCustomValidity("");
  } else {
    confirmationPassword.setCustomValidity("Password didn't match.");
  }
  confirmationPassword.reportValidity();
}
